...
/*   Updated: 2020/07/19 18:01:37 by lchantel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ft_printf_out.h"

void	ft_stdout_int(char space_char[3], char **space_info, int dot_zero_num, 
		char sp_total_info[3])
{
	...
	if (value < 0 && space_char[2] == ' ' && **(space_info + 1) != 'u')
		ft_putchar_fd('-', 1);
	if ((dot_zero_num -= sp_total_info[2]) > 0)
		ft_print_spaces(dot_zero_num, '0');
	if (**(space_info + 1) != 'u')
...
int		ft_integer_handle(char **space_info, int value, char space_char[3])
{
	...
	dot_zero_num = (!**(space_info + 2)) ? 0 :ft_atoi(*(space_info + 2));
	str_value = (**(space_info + 1) != 'u') ? ft_itoa(sp_total_info[1]) : ft_utoa((unsigned int)value);
	if (!str_value)
		return (-1);	
	sp_total_info[2] = ft_strlen(str_value);
	sp_total_info[0] += ((dot_zero_num - sp_total_info[2]) > 0) ? sp_number - dot_zero_num 
	: sp_number - sp_total_info[2];
	ft_stdout_int(space_char, space_info, dot_zero_num, sp_total_info);
