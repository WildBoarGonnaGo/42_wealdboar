...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/28 00:42:53 by lchantel          #+#    #+#             */
/*   Updated: 2020/07/28 01:02:53 by lchantel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/ft_printf_out.h"

void		ft_int_handle_shortcut(int *sp_total_info, char space_char[3],
			char **space_info, int value)
{
	sp_total_info[0] = (space_char[0] == '+' && value > 0) ? -1 : 0;
	sp_total_info[1] = value;
	if ((value < 0 && **(space_info + 1) == 'd')
	|| (value < 0 && **(space_info + 1) == 'i'))
	{
...
int			ft_stdout_int(char space_char[3], char **space_info,
			int value, int sp_total_info[4])
{
	int len_res;
...
int			ft_integer_handle(char **space_info, int value,
			char space_char[3], int *len_res)
{
	...
	ft_int_handle_shortcut(sp_total_info, space_char, space_info, value);
	sp_number = ft_atoi(*space_info);
	sp_total_info[3] = (!**(space_info + 2)) ? 0 : ft_atoi(*(space_info + 2));
	str_value = (**(space_info + 1) != 'u') ?
	ft_itoa(sp_total_info[1]) : ft_utoa((unsigned int)value);
	if (!str_value)
		return (-1);
	sp_total_info[2] = ft_strlen(str_value);
	sp_total_info[0] += ((sp_total_info[3] - sp_total_info[2]) > 0)
	? sp_number - sp_total_info[3] : sp_number - sp_total_info[2];
