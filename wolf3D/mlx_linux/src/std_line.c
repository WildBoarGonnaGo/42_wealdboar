...
/*   Updated: 2020/10/22 02:17:01 by lchantel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			line_output(t_img_info *line_img,
				t_line trgt, unsigned int color)
{
	int pyx;

	pyx = 0;
	trgt.alpha = (trgt.x_end == trgt.x_strt) ? M_PI / 2
	+ (trgt.y_end < trgt.y_strt) * M_PI : atan2(
	(double)(trgt.y_end - trgt.y_strt), (double)(trgt.x_end
	- trgt.x_strt));
	trgt.l = sqrt(pow((trgt.y_end - trgt.y_strt), 2)
	+ pow((trgt.x_end - trgt.x_strt), 2));
	...
	while (trgt.dl <= trgt.l)
	{
		...
		line_img->addr = mlx_get_data_addr(line_img->img,
		&line_img->bits_per_pixel, &line_img->line_size, &line_img->endian);
