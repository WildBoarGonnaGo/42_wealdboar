...
/*   Updated: 2020/10/17 07:24:30 by lchantel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/maze3d.h"

void			item_vars_init(t_raycast *obj_gl, double trans_factor, int i,
				double *old_value)
{
	...
	obj_gl->new_cs_pos[1] = trans_factor * (-old_value[0] * obj_gl->plane_vctr[1]
	...
	> obj_gl->width - 1) ? obj_gl->width - 1 : obj_gl->pxl_itm_xpos + obj_gl->txtr_itm_wratio / 2;
...
void			item_pxl_output(t_raycast *obj_gl)
{
	...
	if (obj_gl->clr_general.red && obj_gl->clr_general.blue && obj_gl->clr_general.green)
	{
		obj_gl->clr_wall_draw = add_shade(0.5, obj_gl->clr_general);
		*(unsigned int *)(obj_gl->img_rndr.addr + obj_gl->wall_ceil * obj_gl->img_rndr.line_size + 
		obj_gl->x_stripe * obj_gl->img_rndr.bits_per_pixel / 8) = obj_gl->clr_wall_draw.clrfull;	
	}
...
void			line_cycle(t_raycast *obj_gl, double *old_value)
{
	...
	* obj_gl->item_txtr.width / obj_gl->txtr_itm_wratio;
	...
	obj_gl->x_stripe < obj_gl->width && obj_gl->z_buffer_tank[obj_gl->x_stripe] > obj_gl->new_cs_pos[1])
...
void			draw_item_text(t_raycast *obj_gl)
{
	...
	{
		item_vars_init(obj_gl, trans_factor, i, old_value);
